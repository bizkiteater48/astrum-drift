...
const registerSchema = authSchema.extend({
  accessCode: z.string().min(1, "Tester access code is required"),
});
...
export default function AuthPage() {
  ...
  const registerForm = useForm<RegisterValues>({
    ...
    defaultValues: { username: "", password: "", accessCode: "" },
  });
  ...
  const onLogin = async (data: AuthValues) => {
    ...
    try {
      ...
      if (error instanceof ApiError && error.status === 403) {
        ...
        setAuthError(
          extractErrorMessage(error) ?? "Failed to authenticate connection",
        );
      }
    }
  };
  ...
  const onRegister = async (data: RegisterValues) => {
    ...
    try {
      ...
      setAuthError(
        extractErrorMessage(error) ??
          "Failed to initialize new commander profile",
      );
    }
  };
  ...
  if (meLoading) {
    return (
      <div className="min-h-[100dvh] nebula-bg flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <p className="text-primary mt-4 font-mono uppercase tracking-widest animate-pulse z-10">
    ...
    <div className="min-h-[100dvh] nebula-bg flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md z-10 space-y-8">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-5xl font-bold uppercase tracking-[0.25em] text-primary text-glow font-mono">
            Astrum Drift
          </h1>
        ...
        >
          <TabsList className="grid w-full grid-cols-2 glass-panel h-12 p-1">
            <TabsTrigger
              value="login"
              className="font-mono uppercase tracking-wider data-[state=active]:bg-primary/20 data-[state=active]:text-primary"
            >
              Initialize
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value="login" className="mt-4">
            <Card className="glass-panel">
              <CardHeader>
                <CardTitle className="font-mono uppercase tracking-widest text-primary text-lg">
                  Commander Login
                </CardTitle>
                <CardDescription className="font-mono text-xs">
                  Enter your credentials to access the ship terminal.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Form {...loginForm}>
                  ...
                  >
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-mono uppercase text-primary/80">
                            Designation
                          </FormLabel>
                          <FormControl>
                            <Input
                              placeholder="CMDR-XYZ"
                              {...field}
                              className="font-mono border-primary/30 focus-visible:ring-primary/50 bg-background/40 text-foreground"
                            />
                          </FormControl>
                    ...
                    {authError && (
                      <div className="bg-destructive/10 border border-destructive/30 p-3 mt-4 rounded-md">
                        <p className="text-destructive text-xs font-mono uppercase font-bold text-glow">
                          {authError}
                        </p>
                      </div>
                    )}
          ...
          <TabsContent value="register" className="mt-4">
            <Card className="glass-panel">
              <CardHeader>
                ...
                <CardDescription className="font-mono text-xs">
                  Register a new profile in the company database.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Form {...registerForm}>
                  ...
                  >
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="font-mono uppercase text-primary/80">
                            Tester Authorization Code
                          </FormLabel>
                          <FormControl>
                            <Input
                              type="password"
                              placeholder="Outpost access code"
                              {...field}
                              className="font-mono border-primary/30 focus-visible:ring-primary/50 bg-background/40 text-foreground"
                            />
                          </FormControl>
